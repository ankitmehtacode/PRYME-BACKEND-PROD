...
def generate_text_files():
    ...
    # -------------------------------------------------------------
    md_path = "/Users/manishmehta/Documents/PRYME-BACKEND-PROD/scratch/PRYME_Eligibility_Engine_Test_Scenarios.md"
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# PRYME Eligibility Engine Test Scenarios Suite\n\n")
        f.write("This document lists 105 structured scenarios to verify every validation logic and edge case in the engine.\n\n")
        
        # Write Overview Table
        f.write("## 1. Quick Reference Matrix\n\n")
        f.write("| ID | Verify Objective | Lender | Product | Employment | CIBIL | Income | Amount | Tenure | Age | Decision | Expected Logic / Reason |\n")
        f.write("| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |\n")
        for sc in scenarios:
            f.write(f"| {sc['id']} | {sc['objective']} | {sc['lender']} | {sc['product']} | {sc['employment']} | {sc['cibil']} | {sc['income']} | ₹{sc['amount']:,} | {sc['tenure']} | {sc['age']} | **{sc['decision']}** | {sc['reason']} |\n")
            
        f.write("\n\n## 2. Copy-Pasteable JSON Payloads\n\n")
        f.write("Below are the JSON request bodies mapped by Scenario ID:\n\n")
        for sc in scenarios:
            f.write(f"### Scenario {sc['id']}\n")
            f.write(f"**Objective**: {sc['objective']}  \n")
            f.write(f"**Expected Decision**: {sc['decision']} ({sc['reason']})  \n\n")
            f.write("```json\n")
            f.write(json.dumps(sc["payload_template"], indent=2))
            f.write("\n```\n\n---\n\n")
            
    print(f"Markdown file successfully generated at: {md_path}")
    
    # -------------------------------------------------------------
    # 3. WRITE CSV FILE
    # -------------------------------------------------------------
    csv_path = "/Users/manishmehta/Documents/PRYME-BACKEND-PROD/scratch/PRYME_Eligibility_Engine_Test_Scenarios.csv"
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "Scenario ID", "Verify Category", "Verify Objective", "Lender", "Product Type",
            "Employment Type", "CIBIL", "Income / Inputs", "Loan Amount", "Tenure (Months)",
            "Age (Years)", "Expected LTV", "Expected FOIR", "Expected Decision", "Expected Logic & Reason"
        ])
        for sc in scenarios:
            writer.writerow([
                sc["id"], sc["category"], sc["objective"], sc["lender"], sc["product"],
                sc["employment"], sc["cibil"], sc["income"], sc["amount"], sc["tenure"],
                sc["age"], sc["expected_ltv"], sc["expected_foir"], sc["decision"], sc["reason"]
            ])
            
    print(f"CSV file successfully generated at: {csv_path}")
